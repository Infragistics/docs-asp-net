...
The {ProductName} product includes a new feature named Persistence Framework. This framework provides the functionality to save the state of the controls on your page and reload them later. The status of the page is persisted during navigation through the pages in a site, if the page is closed, and even if it is opened in a new browser. Persistence Framework allows saving all individual controls and control properties, except those listed below in the <<ignored_controls,Ignored Controls and Properties>> paragraph. You can adjust the framework to persist all the pages or save only certain ones.
