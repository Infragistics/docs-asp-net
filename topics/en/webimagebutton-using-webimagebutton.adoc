...
The first thing to notice is the image URL's which have been set under the various image URL properties. Note that these url's are relative to the ImageDirectory property of the control. By default this is `/ig_common/images/`, and if you have run the {ProductName} installer, this is a virtual directory which is mapped to `C:\inetpub\wwwroot\aspnet_client\infragistics\images` (by default). Browse to that folder in Windows Explorer and open the image files to see what images are actually being used to generate this button's appearance.
