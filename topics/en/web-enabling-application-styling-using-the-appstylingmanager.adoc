...
[start=4]
. Right-click the ig_res folder and select Add Existing Item…. Navigate to the following folder where the StyleSets are installed by default: {InstallPathXP}{PlatformName}\StyleLibraries. Copy all the CSS files and images, and paste them into the corresponding folders created in the preceding steps.
...
'Application Styling for {ProductName} AJAX Controls
...
[NOTE]
====
Setting the properties of either AppStylingManager or AppStylingManagerDesign will result in styling all {ProductName} controls.
====

[start=6]
. Run your application. You will now see the Office2007Black style applied to all the {ProductName} controls in your project.