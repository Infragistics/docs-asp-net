...
In the client side, you may need to retrieve a reference to an HTML element on the page. Most {ProductName} controls expose client-side events which pass in a reference to the HTML element that fired the event; however, what if you needed a reference to another HTML element on the page?
