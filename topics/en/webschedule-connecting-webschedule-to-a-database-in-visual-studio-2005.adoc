...
[start=1]
. Starting with a new Web application, and drag the controls in the following list from the Toolbox onto the design surface. The "ASP.NET tab" will be whichever tab you had chosen to contain the {ProductName} controls and components, including the WebSchedule controls/components.
[start=2]
...
Right-click on the App_Data folder and select Add Existing Item…. You can then navigate to the {ProductName} samples installation folder containing the WebSchedule2.mdb file (or your own Access database file).
