...
[NOTE]
====
If you save the custom language files outside of the /ig_common/Scripts directory, you may need to grant your ASP.NET user identity (this may be the ASP.NET, Network Service, or an impersonated user account, depending on the Web.config settings, and the version of IIS and.NET Framework on which {ProductName} is running) adequate access rights to open and read the custom language file.
====
