...
The AJAX Indicator displays a wait notification for the end users, while an {ProductName} AJAX control is waiting for a callback to complete. The indicator will show itself when a control initiates an AJAX call, and automatically hides itself once the callback is complete. AJAX Indicator can be configured to alert the end user in the form of an image or in html. By default, a shared Ajax Indicator exists for all Infragistics™ AJAX controls with common property settings.
