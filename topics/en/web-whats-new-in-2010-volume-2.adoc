...
The {ProductName} 2010 Volume 2 release includes the following new controls and features built on the {ProductName} AJAX framework.
...
The WebCaptcha™ control is a new addition to the expanding list of {ProductName} AJAX controls. The WebCaptcha component’s design makes it an efficient and easy to use spam protection control, with native AJAX capabilities.

Like all {ProductName} AJAX controls, WebCaptcha seamlessly integrates into the Infragistics® Application Styling framework. With CSS based properties, you can manually customize the WebCaptcha control by leveraging your existing style sheets.
...
The new WebScriptManager control enables the javascript IntelliSense for {ProductName} Ajax controls and provides CDN support.
...
{ProductName} controls now supports Windows 7 Styles.
...
Click on the following link to learn how {ProductName} controls are compatible with ASP.NET 4:
