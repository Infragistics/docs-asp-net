...
You should now be brought to the Query Window. From the File menu, select Open and then choose the SQL install script installed with {ProductName}. This install script loads and you can make changes to it in the Query Window before pressing F5 to Execute the batch commands in the SQL install script.
