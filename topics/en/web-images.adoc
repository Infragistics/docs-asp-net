...
When dealing with the images, one thing to keep in mind, is that the browser must be provided an explicit path to the image file. The {ProductName} components make use of the following three ways to define a path to an image:
