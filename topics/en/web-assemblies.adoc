...
The following table lists all the ASP.NET assemblies included in the {ProductName} product suite. The table also specifies the ASP.NET controls and components included in each assembly, and includes a cross-reference to the resource strings help topic that contains a list of all the resource strings for the assembly.
