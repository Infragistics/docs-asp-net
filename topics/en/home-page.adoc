...
Before you start using {ProductName}, be sure to read through the topics in this section to ensure that you are getting the most of out of the product.

* <<ASP_Using_The_Help,Using the Help>>
* link:web-application-styling-framework-asf.html[Application Styling Framework (ASF)]
* <<ASP_Installing_NetAdvantage,Installing {ProductName}>>
...
The {ProductName} help is available to assist you in finding the information that you need to understand and use the controls and components of the {ProductName}.


.Note:
[NOTE]
====
The Help Center is no longer installed as part of the {ProductName} product. For help on how to use {ProductName}, please refer to the installed/online help, or the ASP.NET Samples Browser.
====

=== Tabs

In the installed {ProductName} help, the bottom of the the left-hand pane of the help window is comprised of the following tabs:
...
In the online {ProductName} help, the left-hand pane is comprised of the Contents and Search tabs only.

=== Filters

Since we provide such an extensive help, we've provided filtering in the installed {ProductName} help to make it easier to find the information that you're looking for. The filters allow you to narrow down the results displayed in the Contents and Index tabs.
...
==== Installing {ProductName} on Windows Vista

Starting with the {ProductName} 2006 Volume 3 (CLR 2.0) release, you can install {ProductName} on the Microsoft® Windows® Vista operating system. Read further for important information about successfully installing {ProductName} on Vista.

* Vista is supported only on Visual Studio 2005 and later versions of Visual Studio; is not supported on Visual Studio 2003 and earlier. Therefore, all CLR 1.x versions of {ProductName} are not supported on Vista. However, the stand-alone version of the Infragistics AppStylist® product can be installed on Vista.

==== Tips for Installing {ProductName}

Sometimes, software installations don't go exactly as planned. We realize that everyone has a different machine setup and we've put together a list of troubleshooting tips that can help ensure that {ProductName} installs as smoothly as possible.

* You must be logged in as an Administrator to install {ProductName}. This login must also be able to update the system registry.
...
* Microsoft® Visual Studio® must be installed and kept closed while installing {ProductName} and/or any of the {ProductName} utilities (e.g. Toolbox utility)..

* The install should be run from a local hard drive. You might encounter issues if it is run from a network drive.
* If you have already installed the trial version of {ProductName}, there is no need to uninstall/reinstall the software. You can simply run the installer file from the appropriate directory (e.g., {InstallPath}) and insert your CD-Key when prompted by the installation wizard.
...
* If you install any combination of the {ProductName} installs and remove one of them at a later time, this might result in known issues. To resolve these issues, reinstall the appropriate {ProductName} product.
* If IIS is installed on Windows® Vista™, you need to follow specific steps prior to running the {ProductName} installer. These steps are documented in the Infragistics Knowledge Base article link:http://devcenter.infragistics.com/Support/KnowledgeBaseArticle.aspx?ArticleID=9983[Installing {ProductName} on Windows Vista]. After following these steps, you should be able to successfully install {ProductName}.
* If you have Infragistics AppStylist for ASP.NET installed, and you modify one of the installed Infragistics Style Library ($$*$$.isl) files or Web Style Library ($$* $$.wsl) files, respectively, and want to save the changes for later, ensure that you save your changes as a different file. This will ensure that your customized .isl or .wsl files are not overwritten by a {ProductName} hotfix, or removed during a {ProductName} uninstall.
...
This folder contains all the in-box presets provided by Infragistics for all {ProductName} controls and components. You can use a preset to apply a range of formatting and behavior options across the elements of your interface and applications.
...
C:\Documents and Settings\[USERID]\MyDocuments\Infragistics\20{ProductVersion}\ASP.NET\StyleLibraries

*In XP - 'All Users' option selected*

C:\Documents and Settings\AllUsers\Documents\Infragistics\20{ProductVersion}\ASP.NET\StyleLibraries

*In Vista* {InstallPathVista}{PlatformName}\StyleLibraries
...
By default when you install {ProductName}, the control assemblies are installed into the GAC.

For more information on installing {ProductName}, see Deploying Your Application section in the ASP.NET help.
...
The easiest way to add all of the tools available in the {ProductName} toolkit to your Visual Studio .NET toolbox is to run the Create Visual Studio Toolbox Tab utility.
...
[start=3]
. The utility installs the {ProductName} controls and components in the Visual Studio toolbox. Keep in mind that the installation of the controls and components is dependent on the {ProductName} product that is installed. For example, if you install only the Windows Forms product, the utility installs only the Windows Forms controls and components. If you install both Windows Forms and ASP.NET products, the utility installs the controls and components for both products.
[start=4]
...
As an alternative to Running the Toolbox Utility to add {ProductName} controls/components to the Visual Studio .NET toolbox, you can add the tools manually using the Choose Toolbox Items dialog box.

===== To manually add {ProductName} controls and components:
...
As an alternative to Running the Toolbox Utility to add {ProductName} controls/components to the Visual Studio .NET toolbox, you can drag Infragistics' assemblies from your Bin folder to the Microsoft® Visual Studio® toolbox.
...
[start=3]
. Give the tab a meaningful name such as "{ProductName}", you may also want to add the volume number (e.g., 8.1, 7.3, 7.2, 7.1) as a suffix.
[start=4]
. Repeat steps 2 and 3 for all the tabs you want to create.
[start=5]
. In Windows Explorer, navigate to the Bin directory where the {ProductName} assemblies are installed. The assemblies contain the DLLs that you want to add:

** ASP.NET -- {InstallPath}\CLR3.5\Bin

[start=6]
. Select the DLLs that contain the {ProductName} controls/components you want to add to the Visual Studio toolbox.
[start=7]
...
When using the shortcut off the Programs menu to launch the {ProductName} product samples, you may receive a "Page not found" error. The reason for this error is that the page is trying to load before the local IIS server has finished loading.
...
* Shut down the IIS server, and launch the {ProductName} samples using the shortcut. This should resolve the issue because the IIS server loads faster after it is first loaded.
...
If you own a copy of {ProductName}, you are entitled to certain benefits regarding support services offered by Infragistics.
