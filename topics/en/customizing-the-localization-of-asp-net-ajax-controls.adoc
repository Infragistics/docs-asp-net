...
The steps below create a satellite assembly for the German (Switzerland) de-CH culture.  The steps can be used for any culture as long as "de-CH" is replaced with a valid link:http://msdn.microsoft.com/en-us/library/ms533052(v=vs.85).aspx[culture code].  The example uses the WebDataGrid, but the same approach applies to any {ProductName} AJAX control.

=== Prerequisites

* The {ProductName} product should be installed.
...
2. Download the source code for the {ProductName} product from the link:https://www.infragistics.com/my-account/keys-and-downloads[Keys and Downloads] section of the link:http://www.infragistics.com[Infragistics website].
