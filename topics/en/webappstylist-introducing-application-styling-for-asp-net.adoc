...
Each {ProductName} control/component has several states that they can exist in. For example, a button can be in its normal state, a hovered state, or even a pressed state. Each of these states is represented by a class in the style sheet - and each control is represented by an entire style sheet.