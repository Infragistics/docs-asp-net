...
デジタル ゲージでより効率的に作業をするために、理解する必要がある一組のプロパティがあります（ pick:[asp-net="link:{ApiPlatform}webui.ultrawebgauge{ApiVersion}~infragistics.ultragauge.resources.segmenteddigitalgauge~mode.html[Mode]"]  と  pick:[asp-net="link:{ApiPlatform}webui.ultrawebgauge{ApiVersion}~infragistics.ultragauge.resources.segmenteddigitalgauge~unlitbrushelement.html[UnlitBrushElement]"] ）。

image::images/Gauge_Working_with_Digital_Gauges_01.png[UnlitBrushElement とモード領域を示すデジタル ゲージ]
