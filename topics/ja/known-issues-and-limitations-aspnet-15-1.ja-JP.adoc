...
|metadata|
{
    "name": "known-issues-and-limitations-aspnet-15-1",
    "controlName": [],
    "tags": [],
    "guid": "7d7205a7-6634-4b7c-99ff-b993479f05c5",
    "buildFlags": [],
    "createdOn": "2015-03-15T20:34:44.6797822Z"
}
|metadata|
////

= ASP.NET 15.1 既知の問題と制限

既知の問題点と制限の概要

=== 既知の問題点と制約の概要表

以下の表に、Infragistics® ASP.NET 2015.1 リリースの既知の問題と制限事項の概要を示します。以下の表は、すべての問題の詳細な説明とその回避策を示します。
...
|非バインド列があり、OnInitializeRow イベントが処理した場合、 _ページングおよび並べ替え機能は正しく操作しません。_ 
|image::images/noworkaround.png[]
...
|RowEditTemplate が WebTextEditor を含み、行を追加するために手動的に開いた場合、WebTextEditor の値は編集の後にクリアされません。結果的として編集モードに入る場合に、以前の値は保持されます。
|image::images/workaround.png[]
...
|_ _HierarchicalDataGrid_ _ の行は Firefox で深さが 200 レベルを超える場合描画されません。
|image::images/noworkaround.png[]
...
|WebDataTree を WebTab 内に使用する場合、ViewState は破損されます。
|WebDataTree コントロールを WebTab 内で使用する場合、ViewState は破損されます。 
