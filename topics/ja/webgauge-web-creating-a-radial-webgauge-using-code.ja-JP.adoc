...
[start=1]
. コードの記述を開始する前にコード ビハインドに using/Imports のディレクティブを配置しますImportsImportsメンバは完全に記述された名前を常にタイプする必要がなくなります。
...
radialScale1 を SScalesコレクションに追加します。
...
** *StartColor* --Gray
** EndColor -- WhiteSmoke
** グラデーション スタイル -- Backward DiBackwardDiagonal
** *RadiusMeasure* -- Percent

次に針を MarkMarkersションに追加します。
...
[start=12]
. スケールを ScaleScalesョンに、ゲージを Gauges コGauges追加しGaugesの以下のプロパティを設定します。
...
image::images/Gauge_Web_Creating_a_Radial_Gauge_Using_Code_01.png[]