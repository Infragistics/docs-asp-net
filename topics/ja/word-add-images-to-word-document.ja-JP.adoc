...
Infragistics® Word ライブラリは、 link:{ApiPlatform}webui.documents.io{ApiVersion}~infragistics.documents.word.worddocumentwriter.html[WordDocumentWriter] ストリーマー オブジェクトを使用して、新しい Word 文書に追加できる 2 種類の画像をサポートします。それらはアンカー固定画像とインライン画像（ピクチャーとも言う）です。
...
インライン画像を段落に追加するには、WordDocumentWriter オブジェクトの link:{ApiPlatform}webui.documents.io{ApiVersion}~infragistics.documents.word.worddocumentwriter~addinlinepicture.html[AddInlinePicture] メソッドを使用します。
