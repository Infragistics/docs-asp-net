...
[start=5]
. [プロパティ] パネルで、 link:webgauge-appearance-tab.html[[外観] タブ]をクリックします。

image::images/Gauge_Modifying_and_Saving_a_Preset_01.png[]

[start=6]
. link:webgauge-brush-pane.html[[ブラシ] ペイン]で、右手のドロップダウン リストを選択することによって、既存のブラシ エレメントの色を編集します。ブラシ エレメントの詳細については、 link:webgauge-apply-brush-elements-using-the-gauge-designer.html[「ゲージ デザイナを使用してブラシ エレメントを適用」]を参照してください。
[start=7]
...
[start=10]
. link:webgauge-stroke-pane.html[[ストローク] ペイン]で、左上のドロップダウン リストから [なし] を選択します。
